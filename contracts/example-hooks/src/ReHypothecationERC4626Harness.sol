...
/// @dev A minimal ERC-4626 yield source. With no external donations its
/// convertToAssets is 1:1, so by default rehypothecation earns no extra yield —
/// the simulator can credit yield by transferring tokens into the vault to model
/// an APY (see the rehypothecation adapter).
contract ERC4626YieldSourceMock is ERC4626 {
    constructor(IERC20 token) ERC4626(token) ERC20("ERC4626YieldSourceMock", "E4626YS") {}
}

/**
 * @dev Concrete, deployable ERC-4626 instantiation of OpenZeppelin's abstract
 * {ReHypothecationHook}, vendored for the simulator as the worked Tier-2 example
 * (a hook that OWNS the LP's liquidity rather than letting the harness place a
 * standard router position). This is OpenZeppelin's own `ReHypothecationERC4626Mock`
 * (src/mocks/general/), with the coverage-only `test()` stub removed and import
 * depth adjusted; behaviour is unchanged.
 *
 * The LP holds this contract's ERC-20 shares; `previewRedeem(shares)` returns the
 * token amounts those shares are worth (including any vault yield), which is how
 * the simulator values an own-liquidity position.
 */
contract ReHypothecationERC4626Mock is ReHypothecationHook {
    ...
    /// @dev Exposed view: assets currently held for `currency` in its yield source.
